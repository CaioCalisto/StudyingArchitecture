...
namespace Customer.Register.Api.Controllers
{
    ...
    public class CountriesController : ControllerBase
    {
        ...
        #region GET
        [HttpGet("{offset}/{next}")]
        public async Task<ActionResult<PaginatedResult<Country>>> Get(int offset, int next)
        {
            try
            {
                offset = offset == 0 ? 0 : offset;
                next = next == 0 ? 10 : next;
                PaginatedResult<Country> result = await countryQueries.GetCountriesAsync(offset, next);
        ...
        public async Task<ActionResult<PaginatedResult<County>>> GetCountiesByCountry(int countryId, int offset, int next)
        {
            try
            {
                PaginatedResult<County> counties = await countryQueries.GetCountiesAsync(countryId, offset, next);
                return Ok(counties);
            }
            catch (Exception ex)
            {
